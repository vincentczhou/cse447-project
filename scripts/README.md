...
#    Open notebooks/plot_sweep.ipynb and run all cells
```
...
uv run python scripts/gemini_predictor.py \
    --input data/madlad_multilang_clean_15k_optionB_kenlm/input_valid.txt \
    --answer data/madlad_multilang_clean_15k_optionB_kenlm/answer_valid.txt \
...
uv run python scripts/parse_predictions_table.py \
    ...
    --output-input work/distill_input.txt
...
uv run python scripts/extend_inputs.py \
    ...
    --output-prefix work/distill_ext

# 4. Append extended inputs to train.txt
uv run python scripts/augment_train.py \
    --train data/madlad_multilang_clean_15k_optionB_kenlm/train.txt \
    --ext work/distill_ext_1.txt work/distill_ext_2.txt work/distill_ext_3.txt
```
...
Appends tokenized extended-input lines to an existing `train.txt`. Each line from the provided `--ext` files is normalized and character-tokenized (matching `preprocess.py` format) before being written `--repeat` times.
